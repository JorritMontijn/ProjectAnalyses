...
for intRec=1:numel(vecUseRec)
	...
	for intRandomize = vecRandomize
		...
		for intArea=1:intAreas
			...
			fprintf('Preparing %d/%d: %s, area %s [%s]\n',intRec,numel(vecUseRec),strRec,strArea,getTime);
			...
			if boolMakeFigs
				...
				if boolSaveFigs
					%% save fig
					export_fig(fullpath(strFigurePath,sprintf('2Cc1O_PopRespPrediction_%s.tif',strRec)));
					export_fig(fullpath(strFigurePath,sprintf('2Cc1O_PopRespPrediction_%s.pdf',strRec)));
				end
			end
		...
		imagesc(matB)
			...
			if boolMakeFigs
				...
				if boolSaveFigs
					%% save fig
					export_fig(fullpath(strFigurePath,sprintf('2Cc2O_Projections_%s.tif',strRec)));
					export_fig(fullpath(strFigurePath,sprintf('2Cc2O_Projections_%s.pdf',strRec)));
				end
			end
			...
			if 0%isfield(sRec,'matPupilXY') && ~isempty(sRec.matPupilXY)
				...
				if boolSaveFigs
					%% save fig
					export_fig(fullpath(strFigurePath,sprintf('2Cc3O_PupilPredGain_%s.tif',strRec)));
					export_fig(fullpath(strFigurePath,sprintf('2Cc3O_PupilPredGain_%s.pdf',strRec)));
				end
			end
