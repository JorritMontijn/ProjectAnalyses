...
strArea = 'Poisson'; %V1, SC, Retina, Poisson, GCaMP
intRunType = 1;
...
%% load data
sDir=dir([strPath 'ZetaData2' strRunType 'Resamp*']);
intFiles=numel(sDir);
...
for intFile=1:intFiles
	...
	if intResampNum < 100026
		...
		vecZeta = abs(sLoad.vecZeta)*(2/5);%/(2/3.5);
...
figure(gcf);
drawnow;
export_fig(sprintf('%sSummary2Fig%s.tif',strFigPath,strRunType));
print(gcf,'-dpdf', sprintf('%sSummary2Fig%s.pdf',strFigPath,strRunType));