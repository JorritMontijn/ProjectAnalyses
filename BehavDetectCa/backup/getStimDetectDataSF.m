function [sContrast,sMetaData] = getStimDetectDataSF(ses,sStimAggregate)
		...
		for intStimType=1:intTypes
			...
			if intPrefNeurons>0
				...
				for intTrial=1:length(vecRespOn)
					%pref + corr
					%matPR(intCounterPR:(intCounterPR+intPrefNeurons-1),:) = std(mean(matActivity(vecPrefNeurons,vecRespOn(intTrial):vecRespOff(intTrial)),2),[],1);
					%dblStimAct = mean(std(matActivity(vecPrefNeurons,vecRespOn(intTrial):vecRespOff(intTrial)),[],2));
					...
					%dblBaseAct = mean(std(matActivity(vecPrefNeurons,vecBaseStart(intTrial):vecBaseStop(intTrial)),[],2));
					...
					%non-pref + corr
					%matNPR(intCounterNPR:(intCounterNPR+intNonPrefNeurons-1),:) = std(mean(matActivity(vecNonPrefNeurons,vecRespOn(intTrial):vecRespOff(intTrial)),2),[],1);
					%dblStimAct = mean(std(matActivity(vecNonPrefNeurons,vecRespOn(intTrial):vecRespOff(intTrial)),[],2));
					...
					%dblBaseAct = mean(std(matActivity(vecNonPrefNeurons,vecBaseStart(intTrial):vecBaseStop(intTrial)),[],2));
