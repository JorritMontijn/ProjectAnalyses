...
%% get all files
sDir=dir([strPath 'ZetaTtestSpDResN*Resamp*.mat']);
cellNames = {sDir(:).name};
...
intSpD = size(cellNumSpikes,3);
matAUC = nan(intTests,intResamps,intSpD);
matAUC_ci = nan(intTests,intResamps,intSpD,2);
cellColor = {[0 0.4 0.8],...
	[0 0 0.5],...
	[0 0 0],...
	[0.5 0 1],...
...
for intTest=1:intTests
	%intTest = intTest + 1;
	matData = sP.(cellTests{intTest});%[resamp x sp-d x n x type]
	for intSpD=1:size(matData,2)
		for intResampIdx=1:intResamps
			intThisResamp = vecUniqueResamps(intResampIdx);
			
			intCells = size(matData,3);
			vecBothData = flat(matData(intResampIdx,intSpD,:,:))';
			vecBothLabels = flat(cat(4,zeros([1 1 intCells 1]),ones([1 1 intCells 1])))';
			vecThresholds = sort(vecBothData);
			vecRealP =matData(intResampIdx,intSpD,:,1);
			vecShuffP = matData(intResampIdx,intSpD,:,2);
			...
			matAUC(intTest,intResampIdx,intSpD) = dblAUC;
			matAUC_ci(intTest,intResampIdx,intSpD,:) = Aci;
		end
		%plot
		plot(vecUniqueResamps,matAUC(intTest,:,intSpD),'color',cellColor{intTest});
		plot(vecUniqueResamps,matAUC_ci(intTest,:,intSpD,1),'--','color',cellColor{intTest});
		plot(vecUniqueResamps,matAUC_ci(intTest,:,intSpD,2),'--','color',cellColor{intTest});
	end
...
if ~isempty(sP) && size(sP.Zeta,1) >= intResampIdx && ~isempty(sP.Zeta{intResampIdx,1})
	...
	for intTest=1:intTests
		%intTest = intTest + 1;
		matData = sP.(cellTests{intTest});
		
		intCells = numel(matData{intResampIdx,1});
		vecBothData = cat(2,matData{intResampIdx,1},matData{intResampIdx,2});
		vecBothLabels = cat(2,zeros(size(matData{intResampIdx,1})),ones(size(matData{intResampIdx,2})));
		vecThresholds = sort(vecBothData);
		vecRealP = matData{intResampIdx,1};
		vecShuffP = matData{intResampIdx,2};
