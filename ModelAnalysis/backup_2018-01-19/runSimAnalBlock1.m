...
for intLoadSim=vecRunSims
	...
	elseif intLoadSim == 211 && boolLoad
		boolLoadLGN = true;
		strSimulation = 'xAlsoLGN_Distrib_Ret100Noise0Ori1Drift2_2018-01-18';
	...
	elseif intLoadSim == 231 && boolLoad
		boolLoadLGN = true;
		strSimulation = 'xAlsoLGN_Distrib_Noise0Drift2Att0_2018-01-16';
		runModelHeader;
		
		strSimulation = 'xAlsoLGN_Distrib_Noise0D3D5Att0_2018-01-19';
	end
	...
	for intWithinArea=[1 2 3]
		...
		if boolAnalI && (intWithinArea == 1 || intWithinArea == 2)
			...
			hFigI = figure;
			hAxI1 = subplot(2,2,1);ylabel('Bias-corrected Fisher information (d''^2)');xlabel('Population size (# of neurons)');title(['Unshuffled; ' strType]);hold on;
			hAxI2 = subplot(2,2,2);ylabel('Bias-corrected Fisher information (d''^2)');xlabel('Population size (# of neurons)');title(['Shuffled; area ' num2str(intWithinArea)]);hold on;
			%full screen
			drawnow;
		...
		for intStimTypeIdx=1:intParamNum
			...
			if boolAnalI && (intWithinArea == 1 || intWithinArea == 2)
				...
				if exist('matModelRespLGN_ON','var')
					%get data
					matModelRespLGN = cat(1,matModelRespLGN_ON,matModelRespLGN_OFF);
					[dblPredA,matPredA,dblD2,dblD2mat,dblD2_diag] = getSeparation(matModelRespLGN,vecTrialStimType,0);
					...
					sParamsInfoLGN.vecGroupSizes = size(matModelRespLGN,1);
					sParamsInfoLGN.intIters = 1;
					sParamsInfoLGN.dblLambda = dblLambdaInfo;
					sParamsInfoLGN.dblDiffTheta = range(vecTrialOris);
					sParamsInfoLGN.vecUseStimTypes = vecUseStimTypes;
					[matFisherFull,sAgg] = doFisherAnal(matModelRespLGN,vecTrialStimType,sParamsInfoLGN);
					%drifting: CV, -0.1; non-CV, 2.6
					%}
					%return
				else
					if isfield(sData,'vecTrialOriNoise')
						dblNoise = mean(sData.vecTrialOriNoise(:));
						dblInputI = (range(vecTrialOris) / sqrt(0.5*(dblNoise^2 + dblNoise^2))).^2;
					else
						dblNoise = 0;
						dblInputI = 0;
					end
			...
			if boolAnalPred
				if ~boolAllZero
					...
					%% get fisher information with full pop
					%matFisherFull = doFisherFull(cellMatX,cellMatY,sParamsAnal);
					
					%% predict with RRR
					matPredDimDepR2 = doDimPredRR(cellMatX,cellMatY);
					matPercOfTotPred = bsxfun(@rdivide,matPredDimDepR2,matPredictionsR2);
					matPredictiveDimensions = sum(matPercOfTotPred<dblCutOff,3);
					...
					%[matSourcePopLogLikeTest,matSourcePopDimensionality] = doDimFactorAnal(cellMatX,dblCutOff,15);
					[matTargetPopLogLikeTest,matTargetPopDimensionality] = doDimFactorAnal(cellMatY,dblCutOff,intMaxDimAnal);
