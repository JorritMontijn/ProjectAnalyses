...
for intRec=19%1:numel(sAggStim)
	...
	for intArea=1:intAreas
		...
		%pre-alloc
		vecCircPrecLow = nan(1,intTrialNum);
		vecCircPrecHigh = nan(1,intTrialNum);
		vecCircPrecLow_shuff = nan(1,intTrialNum);
		vecCircPrecHigh_shuff = nan(1,intTrialNum);
		
		%prep
		vecOri180 = mod(vecOrientation,180)*2;
		matResp = cellfun(@numel,cellSpikeTimesPerCellPerTrial);
		[sOut] = getTuningCurves(matResp,vecOri180,0);
		%indTuned = sOut.vecFitP<0.05 & sum(matResp,2) > 100;
		...
		dblW = 0.1;
		vecBinsE = 0:dblW:dblMaxDur;
		vecBinsC = vecBinsE(2:end)-dblW/2;
		matAggR = nan(numel(vecBinsC),intTrialNum,intTunedN);
		matQ = nan(numel(vecBinsC),intTrialNum);
		for intTrial=1:intTrialNum
			%real
			cellSpikes = cellSpikeTimesPerCellPerTrial(indTuned,intTrial);
			matR = nan(intTunedN,numel(vecBinsC));
			matR_shuff = nan(intTunedN,numel(vecBinsC));
			for intNeuron=1:intTunedN
				matR(intNeuron,:) = histcounts(cellSpikes{intNeuron},vecBinsE);
				matAggR(:,intTrial,intNeuron) = histcounts(cellSpikes{intNeuron},vecBinsE);
				vecISI = diff(cellSpikes{intNeuron});
				vecGenSpikes = cumsum([mean(vecISI);vecISI(randperm(numel(vecISI)))]);
				matR_shuff(intNeuron,:) = histcounts(vecGenSpikes,vecBinsE);
			end
			...
			for intShuff=[0 1]
				if intShuff == 0
					...
					matR_temp = matR_shuff;
				end
				vecR = sum(matR_temp,1);
				
				%get 2nd and 4th quintiles
				dbl5th = numel(vecR)/5;
				indLowIdx = false(size(vecR));
				indLowIdx(round(dbl5th*0+1):round(dbl5th*1)) = true;
				indHighIdx = false(size(vecR));
				indHighIdx(round(4*dbl5th+1):round(5*dbl5th)) = true;
				[vecR_sorted,vecReorder] = sort(vecR);
				vecLowIdx = vecReorder(indLowIdx);
				vecHighIdx = vecReorder(indHighIdx);
				
				matLow = matR_temp(:,vecLowIdx);
				matHigh = matR_temp(:,vecHighIdx);
				
				vecCVLow = nan(1,numel(vecLowIdx));
				vecCVHigh = nan(1,numel(vecHighIdx));
				for intBin=1:numel(vecLowIdx)
					vecSumLow = matLow(:,intBin);
					if sum(vecSumLow) > 1
					...
					vecCVLow(intBin) = 1-dblCircVarLow;
					vecCVHigh(intBin) = 1-dblCircVarHigh;
				end
				
				if intShuff == 0
					matQ(vecLowIdx,intTrial) = 1;
					matQ(vecHighIdx,intTrial) = 2;
					vecCircPrecLow(intTrial) = nanmean(vecCVLow);
					vecCircPrecHigh(intTrial) = nanmean(vecCVHigh);
				else
					vecCircPrecLow_shuff(intTrial) = nanmean(vecCVLow);
					vecCircPrecHigh_shuff(intTrial) = nanmean(vecCVHigh);
				end
		...
		%% decode per bin
		intBinsPerQL = sum(matQ(:,1)==1);
		intBinsPerQH = sum(matQ(:,1)==2);
		matLowR =nan(intTrialNum*intBinsPerQL,intTunedN);
		matHighR =nan(intTrialNum*intBinsPerQH,intTunedN);
		matOri =repmat(vecOri180,[size(matAggR,1) 1]);
		vecOriLow = matOri(matQ==1);
		vecOriHigh = matOri(matQ==2);
		for intNeuron=1:intTunedN
			matTempLow = matAggR(:,:,intNeuron);
			matLowR(:,intNeuron) = matTempLow(matQ==1);
			
			matTempHigh = matAggR(:,:,intNeuron);
			matHighR(:,intNeuron) = matTempHigh(matQ==2);
		end
		
		%% mean per trial per quantile
		matLowR =nan(intTrialNum,intTunedN);
		matHighR =nan(intTrialNum,intTunedN);
		vecOriLow = vecOri180;
		...
		for intNeuron=1:intTunedN
			...
			matTempHigh = matAggR(:,:,intNeuron);
			for intTrial=1:intTrialNum
				matLowR(intTrial,intNeuron) = mean(matTempLow(matQ(:,intTrial)==1,intTrial));
				matHighR(intTrial,intNeuron) = mean(matTempHigh(matQ(:,intTrial)==2,intTrial));
			end
		end
