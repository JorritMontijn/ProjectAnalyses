...
intMakePlots =3; %0=none, 1=normal plot, 2=including raster
vecRunTypes = 1;%[1 2];
boolSave = true;
vecResamples = [10:10:50 100];
vecRunAreas = 1:6;%1:6;%1:5;
...
for intResampleIdx = 1:numel(vecResamples)
	...
	for intNeuron=[1:intNeurons]%31
		...
		if exist('vecTraceAct','var') && ~isempty(vecTraceAct) %calcium imaging
			...
			if intResampleNum == vecResamples(end)% && vecPlotCells(intNeuron)
				[dblZeta,vecLatencies,sZETA,sMSD] = getTraceZeta(vecTraceT,vecTraceAct,matStimOnOff,[],intResampleNum,intMakePlots);
			else
				[dblZeta,vecLatencies,sZETA,sMSD] = getTraceZeta(vecTraceT,vecTraceAct,matStimOnOff,[],intResampleNum,0);
			end
		...
		%% save plot
		if 0%intMakePlots && (intResampleNum == vecResamples(end))% && ~(exist('vecTraceAct','var') && ~isempty(vecTraceAct)))
			%plot
	...
	%cellArea = cell(1,intNeurons);
	if boolSave
		save([strDataTargetPath 'ZetaDataMSD' strRunType 'Resamp' num2str(intResampleNum) '.mat' ],...
			'vecPeakT','vecPeakW','vecNumSpikes','vecZeta','vecP','vecHzD','vecHzP','cellZ','cellInterpT','cellArea','cellDeriv','cellPeakT','cellPeakI');
	end
end
end
end