...
%% set variables
strRec = sprintf('SimBurstsResamp%d',intResampNum);
intNeurons = 100;
...
for intNeuron=1:intNeurons
	...
	dblPrefBurstEventRate = dblBurstEventRate; %mean evoked rate of burst events (Hz) (exponential inter-event times)
	...
	%% generate data
	% generate bursts
	[vecSpikeTimes1,dblPrefOri] = getGeneratedBurstingData(0,[1 max(matTrialT(:))+10],sSpikingParams,sTuningParams);
	
	% generate peak
	dblJitter = 5; %in ms'
	dblExtraSpikeFrac = 1/4;
	intAddSpikes = round(numel(matTrialT(:,1))*dblExtraSpikeFrac);
	[vecSpikeTimes2,dblPrefOri] = getGeneratedSpikingDataWithPeak(vecTrialAngles,matTrialT,0,0,dblJitter,dblKappa,boolDoublePeaked,dblPrefOri,intAddSpikes);
...
if boolSave
	save([strDataTargetPath 'ZetaTime' strRec '.mat' ],...
		'matAnova','matCompTimeAnova','vecTestResamps','matTtest','matZeta','matZetaTime','matCompTimeZeta','matCompTimeZetaTime','intResampNum','intAddSpikes','strRec');
end
