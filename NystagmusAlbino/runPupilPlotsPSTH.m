...
%% get data
if exist(fullpath(strTargetPath,'PupilPSTH.mat'),'file')
	%load
		...
		for intRecIdx=1:numel(vecRunRecs)
			...
			for intMoveType = 1:3
				...
				for intArea=1:intRunMaxArea
					...
					vecZetaP = nan(1,intCellNum);
					for intCell=1:intCellNum
						vecSpikes = cellSpikes{intCell};
						intOrigId = vecSelectCells(intCell);
						[dblZetaP,sZeta] = zetatest(vecSpikes,vecStartMoveT-2,4,intResampNum,intPlot);
						vecZetaP(intCell) = dblZetaP;
					end
