...
%remove cells in wrong areas
indRemoveAreaNeurons = ~contains(cellArea,strArea)';
cellAllAreas = unique(cellArea);
...
matTtest2P(indRemEntries|indRemoveAreaNeurons,:) = [];
matZeta2P(indRemEntries|indRemoveAreaNeurons,:) = [];
matAnova2P(indRemEntries|indRemoveAreaNeurons,:) = [];
cellArea(indRemEntries|indRemoveAreaNeurons) = [];
matTrialNumCorr(indRemEntries|indRemoveAreaNeurons,:) = [];
matTrialNumIncorr(indRemEntries|indRemoveAreaNeurons,:) = [];
vecRecording(indRemEntries|indRemoveAreaNeurons) = [];
...
matAnova2P(indRem,:)=[];
cellArea(indRem)=[];
matTrialNumCorr(indRem,:)=[];
matTrialNumIncorr(indRem,:)=[];
vecRecording(indRem)=[];
...
set(gca,'xscale','log','yscale','log');
dblMinVal = 1e-3;%max(get(gca,'xlim'),get(gca,'ylim'));
