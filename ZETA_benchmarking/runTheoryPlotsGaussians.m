%%
L_s=200; %rate during stimulus
L_b=200; %rate during baseline
T = 1; %stim duration
...
for i_m=1:mNum
	...
	%simulate
	[E_maxDprime_sim,S_maxDprime_sim,var_Dprime_sim,E_maxD_sim,S_maxD_sim,E_zetap_sim]=getD_simulation_GaussianD(L_s,L_b,T,tau,m,intReps);
...
%hybrid
vecEmaxDprime_hyb = nan(1,mNum);
vecS_maxDprime_hyb = nan(1,mNum);
vecvar_Dprime_hyb = nan(1,mNum);
vecE_maxD_hyb = nan(1,mNum);
vecS_maxD_hyb = nan(1,mNum);
vecE_zetaP_hyb = nan(1,mNum);
for i_m=1:mNum
	m=vecM(i_m);
	if toc(hTic) > 5
		m
	...
	%theory
	[E_maxDprime,S_maxDprime,var_Dprime,E_maxD,S_maxD,E_zetap]=getD_theory_GaussianD(L_s,L_b,T,tau,m);
	...
	%hybrid
	[E_maxDprime,S_maxDprime,var_Dprime,E_maxD,S_maxD,dblZetaP]=getD_hybrid_GaussianD(L_s,L_b,T,tau,m,vecvar_Dprime_sim(i_m));
	vecEmaxDprime_hyb(i_m)=E_maxDprime;
	vecS_maxDprime_hyb(i_m)=S_maxDprime;
	vecvar_Dprime_hyb(i_m)=var_Dprime;
	vecE_maxD_hyb(i_m)=E_maxD;
	vecS_maxD_hyb(i_m)=S_maxD;
	vecE_zetaP_hyb(i_m)=E_zetap;
...
plot(vecM,vecS_maxDprime_hyb,'m--');
hold off
title('S[max(d'')]')
ylim([0 max(get(gca,'ylim'))]);
