...
for intPopIdx=1:intPops
	...
	indKeepTrials = indKeepTrials & vecTrialMisses;
	...
	for intIter=1:intFullIters
		...
		matMeanAlignment = nanmean(nanmean(matAlignmentWithX,4),3);
		matMeanMagnitude = nanmean(nanmean(matMagnitudeOfPop,4),3);
		vecNormMeanAlignment = nanmean(matMeanAlignment,2);
...
if boolSavePlots
	strFigFile = ['DimCom3ST2_Misses_Window' sprintf('%.1f',dblBinSizeSecs) 'ProgressionNeuralDynamicsSummary' getDate];
