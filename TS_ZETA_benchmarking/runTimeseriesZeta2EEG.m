...
title(sprintf('Houses-signif, Z: %d%%/%.1f%%, T: %d%%/%.1f%%, R: %d%%/%.1f%%',...
	round(sum(matAllZetaZ_houses(:,1)>1.96)/numel(matAllZetaZ_houses(:,1))*100),...
	(sum(matAllZetaZ_houses(:,2)>1.96)/numel(matAllZetaZ_houses(:,2))*100),...
	round(sum(matAllTtestZ_houses(:,1)>1.96)/numel(matAllTtestZ_houses(:,1))*100),...
	(sum(matAllTtestZ_houses(:,2)>1.96)/numel(matAllTtestZ_houses(:,2))*100),...
	round(sum(matAllR2_houses(:,1)>0.05)/numel(matAllR2_houses(:,1))*100),...
	(sum(matAllR2_houses(:,2)>0.05)/numel(matAllR2_houses(:,2))*100)...
	))
...
title(sprintf('Faces-signif, Z: %d%%/%.1f%%, T: %d%%/%.1f%%, R: %d%%/%.1f%%',...
	round(sum(matAllZetaZ_faces(:,1)>1.96)/numel(matAllZetaZ_faces(:,1))*100),...
	(sum(matAllZetaZ_faces(:,2)>1.96)/numel(matAllZetaZ_faces(:,2))*100),...
	round(sum(matAllTtestZ_faces(:,1)>1.96)/numel(matAllTtestZ_faces(:,1))*100),...
	(sum(matAllTtestZ_faces(:,2)>1.96)/numel(matAllTtestZ_faces(:,2))*100),...
	round(sum(matAllR2_faces(:,1)>0.05)/numel(matAllR2_faces(:,1))*100),...
	(sum(matAllR2_faces(:,2)>0.05)/numel(matAllR2_faces(:,2))*100)...
	))
legend({'ZETA','T-test'},'location','best');
ylabel('ZETA/t-test Significance (\sigma)');
xlabel('Signal predictability (R^2)');
...
title(sprintf('Diff-signif, Z: %d%%/%.1f%%, C: %d%%/%.1f%%; bino2-p=%.3f',...
	round(sum(vecRealZ>1.96)/numel(vecRealZ)*100),...
	(sum(matAllZetaZ_diff(:,2)>1.96)/numel(matAllZetaZ_diff(:,2))*100),...
	round(sum(vecRealC>1.96)/numel(vecRealC)*100),...
	(sum(matAllClustZ_diff(:,2)>1.96)/numel(matAllClustZ_diff(:,2))*100),pBino2...
	))
