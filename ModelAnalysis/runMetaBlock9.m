...
strDataDir = 'D:\Data\Results\Data9\';
...
for intFileIdx=1:numel(sFiles)
	...
	cellRaw{intFileIdx} = matRawFisher;
	cellShuff{intFileIdx} = matShuffFisher;
...
title(['\phi ' sprintf('(AUC) up to subspace rank %d; mean=%.3f; t-test vs 0,p=%.6f',intMaxDim,mean(vecMean),dblP_zero)]);
%title(['\phi ' sprintf('(AUC) up to subspace rank %d; mean=%.3f; t-test vs 0,p=%.6f; t-test vs baseline (%.3f),p=%.6f',intMaxDim,mean(vecMean),dblP_zero,dblBaseMean,dblP_base)]);
