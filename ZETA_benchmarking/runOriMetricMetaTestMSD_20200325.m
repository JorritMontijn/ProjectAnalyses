...
%set var
strRunType = [cellUniqueAreas{intRunArea} cellRunRand{intRunRandType} cellRunStim{intRunStimType}];

%% load data
sDir=dir([strPath 'ZetaDataMSD' strRunType 'Resamp*']);
intFiles=numel(sDir);
...
for intFile=1:intFiles
	...
	if intResampNum < 100026
		...
		vecZeta(isinf(vecZeta)) = 10;
		vecZP=1-(normcdf(abs(vecZeta))-normcdf(-abs(vecZeta)));
		%cellZeta{intC} = sLoad.cellZ;
		...
		[vecR(intC),vecP(intC)] = nancorr(sLoad.vecNumSpikes',vecZeta');
		...
		intFive = ceil(sum(~isnan(vecZeta) & ~isinf(vecZeta))*0.95);
		vecFivePerc(intC) = vecSortedZ(intFive)
		for intN=1:numel(sLoad.cellD)
			if isempty(sLoad.cellD{intN}),continue;end
			[dummy,intIdx]=max(sLoad.cellD{intN});
			matTmaxD(intC,intN)=intIdx/numel(sLoad.cellD{intN});
		end
	end
...
[vecParams,RESNORM,RESIDUAL,EXITFLAG] = lsqcurvefit(@(p,x) p(3) + p(1)*exp(-p(2)*x), vecP0, vecResampNum, vecFivePerc,[0 0 0],[10000 10 10000]);
...
vecBins = 0:(1/18):1;
subplot(4,2,5);
scatter(matTmaxD(vecResampNum==intUseResamp,:),matZeta(vecResampNum==intUseResamp,:),[],vecColor);
colormap(mapC);
ylabel('Z-score');
...
subplot(4,2,7);
vecTmaxZ = matTmaxD(vecResampNum==intUseResamp,:);
...
export_fig(sprintf('%sSummaryFigEF%s.pdf',strFigPath,strRunType));
