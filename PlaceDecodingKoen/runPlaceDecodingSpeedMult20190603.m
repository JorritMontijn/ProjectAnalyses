...
for intNeuron = 1:intNeurons
	...
	vecTrace = vecY;
	vecTimestamps = vecFrameTimes;
	vecTrialStarts = vecTrialStartTime;
	
	
	[dblZ,vecInterpT,vecZ,matDiffTest,dblHzD,dblP] = getVisualResponsivenessTrace(vecTimestamps,vecTrace,vecTrialStarts,true,3);
	pause
	close
end
return
...
for intPredictor=1:intPredictors
	...
	hold all;
	vecCounts = histcounts(matR_squared(:,intPredictor+1),vecBins);
...
figure(gcf);
drawnow;
strFig = ['PredictorRemoved_' strFile '_' getDate];
pause(0.5);
%save
...
for intPredictor=1:intPredictors
	subplot(2,3,intPredictor);
	hold all;
	vecCounts = histcounts(matR2_uniqueContrib(:,intPredictor),vecBins);
	bar(vecBinPlotX,vecCounts);
	ylabel('Number of neurons (count)');
	xlabel('R^2');
	xlim([0 0.6])
...
figure(gcf);
drawnow;
strFig = ['PredictorUniqueContrib_' strFile '_' getDate];
pause(0.5);
%save
export_fig([strFigPath strFig '.tif']);
export_fig([strFigPath strFig '.pdf']);

%% full model and example
vecPlot = 1:5000;
figure
subplot(2,2,1)
...
figure(gcf);
drawnow;
strFig = ['FullPerfExample_' strFile '_' getDate];
pause(0.5);
%save
export_fig([strFigPath strFig '.tif']);
export_fig([strFigPath strFig '.pdf']);
