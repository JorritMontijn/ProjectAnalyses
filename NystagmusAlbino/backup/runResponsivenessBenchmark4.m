...
for intResampleIdx = 1:numel(vecResamples)
	...
	for intNeuron=[1:intNeurons]%31
		...
		if exist('vecTraceAct','var') && ~isempty(vecTraceAct) %calcium imaging
			...
			if intResampleNum == vecResamples(end) && vecPlotCells(intNeuron)
				[dblZeta,sOptOut] = getZetaMSD(vecSpikeTimes,matStimOnOff,2,[],intResampleNum);
			else
				[dblZeta,sOptOut] = getZetaMSD(vecSpikeTimes,matStimOnOff,intMakePlots,[],intResampleNum);
			end
			intSpikeNum = numel(vecSpikeTimes);
			%get smooth deriv
			vecSmoothDeriv = sOptOut.vecRealMSprime;
			%unpack
			vecPeakTimes = sOptOut.vecPeakTimes;
			vecPeakValues = sOptOut.vecPeakValues;
			vecBinT = sOptOut.vecSpikeT;
			vecLDD = vecSmoothDeriv;
			vecDerivT = sOptOut.vecSpikeT;
	...
	%cellArea = cell(1,intNeurons);
	if boolSave
		save([strDataTargetPath 'ZetaData2' strRunType 'Resamp' num2str(intResampleNum) '.mat' ],...
