...
strConnType = 'Least';%'Most' or 'Least'
...
for i=1:numel(cellSuffices) %dur, conf, ifr
	...
	if boolSaveFig
		%%
		export_fig(fullpath(strFigurePath,sprintf('M1_SummaryUniformNormalized%s%s%s%s.tif',strRunType,strSGS,strConnType,strOnset)));
		export_fig(fullpath(strFigurePath,sprintf('M1_SummaryUniformNormalized%s%s%s%s.pdf',strRunType,strSGS,strConnType,strOnset)));
	end
end