...
for intMouse=1:8
	...
	for intPopulation = vecBlockTypes
		...
		%plot
		vecContrasts = unique(cellMultiSes{intPopulation}.structStim.Contrast)*100;
	...
	indSelectHitAnticorrelatedNeurons
		...
		if intHeatMapType == 2
			...
			for intTrial=1:intTrials
				...
				if indStimResp(intTrial)
					cellHitPrefAct{intContrastIndex} = [cellHitPrefAct{intContrastIndex};vecPrefAct];
					cellHitNonPrefAct{intContrastIndex} = [cellHitNonPrefAct{intContrastIndex};vecNonPrefAct];
				else
					cellMissPrefAct{intContrastIndex} = [cellMissPrefAct{intContrastIndex};vecPrefAct];
					cellMissNonPrefAct{intContrastIndex} = [cellMissNonPrefAct{intContrastIndex};vecNonPrefAct];
				end
			end
			...
			if intSwitchZ~=1
				...
				for intContrast=1:intContrasts
					matHit(:,intContrast) = matHit(:,intContrast) + hist([cellHitPrefAct{intContrast};cellHitNonPrefAct{intContrast}],vecBins)';
					matMiss(:,intContrast) = matMiss(:,intContrast) + hist([cellMissPrefAct{intContrast};cellMissNonPrefAct{intContrast}],vecBins)';
				end
			...
			cellHitPrefAct = cell(intContrasts,intRepRecs);
			cellMissPrefAct = cell(intContrasts,intRepRecs);
			for intRepRec=1:intRepRecs
				...
				for intContrast=1:intContrasts
					...
					cellHitPrefAct{intContrast,intRepRec} = nan(1,intNeurons*length(ses.structStim.Orientation));
					cellMissPrefAct{intContrast,intRepRec} = nan(1,intNeurons*length(ses.structStim.Orientation));
					for intNeuron=1:intNeurons_sba
						...
						cellHitPrefAct{intContrast,intRepRec}(intHitTrialCounter_sba:(intHitTrialCounter_sba+sum(indPrefHitTrials)-1)) = vecHitResp;
						cellMissPrefAct{intContrast,intRepRec}(intMissTrialCounter_sba:(intMissTrialCounter_sba+sum(indPrefMissTrials)-1)) = vecMissResp;
					...
					%remove trailing nans
					cellHitPrefAct{intContrast,intRepRec}(isnan(cellHitPrefAct{intContrast,intRepRec})) = [];
					cellMissPrefAct{intContrast,intRepRec}(isnan(cellMissPrefAct{intContrast,intRepRec})) = [];
				end
			end
			%data
			intSwitchZ = 1;
			if intSwitchZ~=1
				...
				for intRepRec = 1:intRepRecs
					for intContrast=1:intContrasts
						matHit(:,intContrast) = matHit(:,intContrast) + hist(cellHitPrefAct{intContrast,intRepRec},vecBins)';
						matMiss(:,intContrast) = matMiss(:,intContrast) + hist(cellMissPrefAct{intContrast,intRepRec},vecBins)';
					end
				end
				strLabelY = sprintf('dF/F0 distribution');
			else
				...
				for intRepRec = 1:intRepRecs
					for intContrast=1:intContrasts
						matHit(:,intContrast) = matHit(:,intContrast) + hist(zscore(cellHitPrefAct{intContrast,intRepRec}),vecBins)';
						matMiss(:,intContrast) = matMiss(:,intContrast) + hist(zscore(cellMissPrefAct{intContrast,intRepRec}),vecBins)';
					end
				end
				strLabelY = sprintf('Z-scored dF/F0 distribution');
			end
		...
		elseif intTestType == 3
			...
			matRealMean = matDiff;
		elseif intTestType == 4
			%shuffle values
			intIters = 1000;
			matShuffledDiff = zeros(size(matDiff,1),size(matDiff,2),intIters);
			for intIter=1:1000
				matShuffled = reshape(matDiff(randperm(numel(matDiff))),size(matDiff));
				matShuffledDiff(:,:,intIter) = conv2(matShuffled,matFilt,'same');
			end
			
			%calc 99% CI
			matP = mean(matShuffledDiff,3);
			dblAlpha = 0.01/prod([size(matDiff,1) size(matDiff,2)]);
			dblSigma = abs(qnorm(dblAlpha));
			matCutOffUp=mean(matShuffledDiff,3)+dblSigma*std(matShuffledDiff,[],3);
			matCutOffDown=mean(matShuffledDiff,3)-dblSigma*std(matShuffledDiff,[],3);
			matH = matRealMean>matCutOffUp | matRealMean<matCutOffDown;
		end
		...
		figure(gcf);
		drawnow;
		strFig = sprintf('%sagg_detectcorrelated_heatmaps_nonpref_pop%d_raw',strSes,intPopulation);
		export_fig([strFig '.tif']);
		export_fig([strFig '.pdf']);
		%end
