...
for intFile=1:intRecNum
	...
	strRec = strRec((1+numel('Q1bData_Rec')):(16+numel('Q1bData_Rec')));
	...
	title([strRunType '; spikes per bin'])
	...
	drawnow;
	export_fig(fullpath(strFigurePathSR,sprintf('Q1%s.tif',strRec,strRunType)));
	export_fig(fullpath(strFigurePathSR,sprintf('Q1%s.pdf',strRec,strRunType)));
	end
	%% save data
...
title(sprintf('%s; Avg spiking per %.1f ms bin, n=%d recs',strRunType,dblBinWidth*1000,intUseRecs))
xlabel('Time after onset (s)');
ylabel('Binned population spiking rate (Hz)');
fixfig;
...
drawnow;
export_fig(fullpath(strFigurePath,sprintf('Q1_DynamicsSpikingDecoding%s.tif',strRunType)));
export_fig(fullpath(strFigurePath,sprintf('Q1_DynamicsSpikingDecoding%s.pdf',strRunType)));
