...
for intType = 1:numel(cellTypes)
	...
	sFiles = dir ([strTargetDataPath 'Q1Data*' strType '.mat']);
	...
	for intFile=1:intRecNum
		...
		if boolSinglePlots
			...
			title(sprintf('Spikes per bin, %s - %s',strRec,strType))
			...
			drawnow;
			export_fig(fullpath(strFigurePathSR,sprintf('Q1%s%s.tif',strRec,strType)));
			export_fig(fullpath(strFigurePathSR,sprintf('Q1%s%s.pdf',strRec,strType)));
		end
		%% save data
	...
	indUseRecs = (vecCorrP<0.01 & vecAvgPerf>0.09) | strcmp(strType,'PoissGain');
	...
	plot([vecStimTime(1) vecStimTime(end)],[dblChance dblChance],'--','color',[0.5 0.5 0.5]);
	hold off
	title(sprintf('%s, Dec perf',strType),'interpreter','none')
	xlabel('Time after onset (s)');
	...
	drawnow;
	export_fig(fullpath(strFigurePath,sprintf('Q1_DynamicsSpikingDecoding%s.tif',strType)));
	export_fig(fullpath(strFigurePath,sprintf('Q1_DynamicsSpikingDecoding%s.pdf',strType)));
end