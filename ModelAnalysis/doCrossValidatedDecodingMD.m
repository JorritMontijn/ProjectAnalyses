function [dblPerformance,vecDecodedIndexCV,matMahalDistsCV,dblMeanErrorDegs,matConfusion] = doCrossValidatedDecodingMD(matData,vecTrialTypes,intTypeCV)
	...
	if intTypeCV == 0
		...
		for intStimType=1:intStimTypes
			vecMu = mean(matData(:,vecTrialTypeIdx==intStimType),2);
	...
	elseif intTypeCV == 1
		...
		for intLeaveOut=1:intTrials
			...
			%calc CV mean
			vecMuCV = mean(matData(:,(vecTrialTypeIdx==intTypeCVTrial)&indSelect),2);
	...
	elseif intTypeCV == 2
		...
		for intRep=1:intRepNum
			...
			for intStimType=1:intStimTypes
				%get distances
				vecMu = mean(matThisData(:,vecThisTrialType==intStimType),2);
