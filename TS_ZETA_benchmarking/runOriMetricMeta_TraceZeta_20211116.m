...
for boolBuildPseudoTrace = [false true]
	...
	for boolUseSuperResolution = [false true]
		
		if boolUseSuperResolution
			strTitSR = 'SR';
		else
			strTitSR = 'NoSR';
		end
		for boolDoOGB = [false true]
			...
			sDirAll1=dir([strPath 'TraceZeta' strIndicator '_' strTitPD strTitSR '*ses' strDur 'Resamp100.mat']);
			sDirAll2=dir([strPath 'TraceZeta' strIndicator '_' strTitPD strTitSR '*ses-Rand' strDur 'Resamp100.mat']);
			...
			title(sprintf('C) %s - %s - %s%s; ROC analysis',strIndicator,strTitSR,strTitPD,strDur));
			legend({sprintf('ZETA-test, AUC=%.2f',vecAUC(1)),sprintf('t-test, AUC=%.2f',vecAUC(2))},'location','best','interpreter','none');
			fixfig;
			
			%% save
			drawnow;
			export_fig(fullpath(strFigPath,['TraceZeta' strIndicator strTitPD strTitSR strDur '.tif']));
			export_fig(fullpath(strFigPath,['TraceZeta' strIndicator strTitPD strTitSR strDur '.pdf']));
			
			%% add to mega fig
			figure(hMegaFig);drawnow;
			intPlot = (boolDoOGB)*4 + (boolUseSuperResolution)*2 + boolBuildPseudoTrace + 1;
			subplot(2,4,intPlot);
			cellColor = {lines(1),'k'};
			%vecH(intResampNpx) = subplot(4,3,intResampNpx);
			hold on;
			...
			xlabel('False positive fraction');
			ylabel('Inclusion fraction');
			title(sprintf('%s - %s - %s%s; ROC analysis',strIndicator,strTitSR,strTitPD,strDur));
			legend({sprintf('ZETA-test, AUC=%.2f',vecAUC(1)),sprintf('t-test, AUC=%.2f',vecAUC(2))},'location','best','interpreter','none');
			fixfig;
...
figure(hMegaFig)
drawnow;
export_fig(fullpath(strFigPath,['TraceZetaSummary' strDur '.tif']));
export_fig(fullpath(strFigPath,['TraceZetaSummary' strDur '.pdf']));
