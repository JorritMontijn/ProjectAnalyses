...
cellPredictors = {'EyePosX','EyePosY','EyeSize','MotLoc','MotSpeed','Licking'};
...
for intNeuron = 1:intNeurons
	...
	for intPredictor=1:intPredictors
		...
		if intPredictor == 4
			[cellTheseCoeffsRem,matTempOutX,cellTempFunctions] = gnmfit(matTempX,vecY,cellTheseCoeffs0,'constant',intPredictors);
		elseif intPredictor < 4
			[cellTheseCoeffsRem,matTempOutX,cellTempFunctions] = gnmfit(matTempX,vecY,cellTheseCoeffs0,'gnmgauss',3,'constant',intPredictors);
		elseif intPredictor > 4
...
vecGoodTime = vecFrameTimes(1:size(matGoodX,1));
%%
figure
matDecodedX = nan(size(matGoodX));
for intPredictor=1:intPredictors
	subplot(2,3,intPredictor);
	hold all;
	vecWeights = matWeights(:,intPredictor);
	vecRealX = matGoodX(:,intPredictor);
	vecDecodedX =  vecWeights(1) + vecWeights(2)*matAct*vecWeights(3:end);
	matDecodedX(:,intPredictor) = vecDecodedX;
	plot(vecGoodTime,vecRealX);
	plot(vecGoodTime,vecDecodedX);
	[dblR2,dblSS_tot,dblSS_res] = getR2(vecRealX,vecDecodedX);
	ylabel(cellPredictors{intPredictor});
	xlabel('Time (s)');
...
%% plot location
vecDecLoc = matDecodedX(:,4);
vecRealLoc = matGoodX(:,4);
