function [E_maxDprime,S_maxDprime,var_Dprime,E_maxD,S_maxD,dblZetaP]=getD_theory_GaussianD(L_s,L_b,T,tau,m)
...
%% s[max(d')]
beta = 1;%4/pi; %correction factor
N_v = N;

V_maxX = ((N_v) ./ (((N_v+1).^2).*(N_v+2))).*(1./((normpdf(norminv(N_v./(N_v+1)))).^2));
S_maxDprime = beta*sqrt(var_Dprime.*V_maxX);
