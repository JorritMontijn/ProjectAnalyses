...
for intRec=16%1:numel(sAggStim)
	...
	for intArea=1:intAreas
		...
		subplot(2,3,3)
		vecEffectLow_R2 = (vecTuningR2Low-vecTuningR2Low_shuff)./(vecTuningR2Low_shuff+vecTuningR2Low);
		vecEffectHigh_R2 = (vecTuningR2High-vecTuningR2High_shuff)./(vecTuningR2High+vecTuningR2High_shuff);
		%plot(dblMaxVal*[0 1],dblMaxVal*[0 1],'k--')
		hold on
		...
		xlabel('d(R2) low q; d(real,shuffled)');
		...
		xlabel('d(R2) high q; d(real,shuffled)');
		ylabel('Number of trials (count)')
		fixfig;
		...
		%plot(dblMaxVal*[0 1],dblMaxVal*[0 1],'k--')
		hold on
		scatter(vecCircPrecHigh_shuff,vecCircPrecHigh,[],vecColH,'.')
		hold off
		title(sprintf('Highest %d%%, real=%.3f, shuff=%.3f',dblQuantileSize,mean(vecCircPrecHigh),mean(vecCircPrecHigh_shuff)))
		ylabel('Real circ prec');
		xlabel('Shuffled circ prec');
		fixfig;
		
		subplot(2,3,3)
		vecEffectLow_CP = (vecCircPrecLow-vecCircPrecLow_shuff)./(vecCircPrecLow_shuff+vecCircPrecLow);
		vecEffectHigh_CP = (vecCircPrecHigh-vecCircPrecHigh_shuff)./(vecCircPrecHigh_shuff+vecCircPrecHigh);
		%plot(dblMaxVal*[0 1],dblMaxVal*[0 1],'k--')
		hold on
