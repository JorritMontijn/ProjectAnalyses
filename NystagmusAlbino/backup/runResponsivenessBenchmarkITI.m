...
intMakePlots = 0; %-1=automatic, 0=none, 1=normal plot, 2=including raster
vecRunTypes = [1 2];
intResampleNum = 100;
boolSave = true;
vecResamples = 50;%[5:5:25 50:25:100 250];

%set var
for intArea=2%1:4%4%3:numel(cellUniqueAreas)
	strArea = cellUniqueAreas{intArea};
	%reset vars
	clearvars -except vecResamples boolSave intArea strArea cellUniqueAreas strDataMasterPath strDataTargetPath strFigPath intMakePlots vecRunTypes
...
for intResampleIdx = 1:numel(vecResamples)
	...
	%% analyze
	for intNeuron=27%[1:intNeurons]%31 27
		%% message
		if toc(hTic) > 5
		...
		%check if this run is necessary
		if intResampleNum > numel(vecStimOnTime)
			continue;
		end
		...
		if exist('vecTraceAct','var') && ~isempty(vecTraceAct) %calcium imaging
			...
			if intResampleNum == vecResamples(end)% && vecPlotCells(intNeuron)
				[dblZeta,sOptOut] = getZetaITI(vecSpikeTimes,matStimOnOff,2,[],intResampleNum,0);
			else
				[dblZeta,sOptOut] = getZetaITI(vecSpikeTimes,matStimOnOff,intMakePlots,[],intResampleNum,0);
			end
		...
		if intMakePlots || (intResampleNum == vecResamples(end))% && vecPlotCells(intNeuron))% && ~(exist('vecTraceAct','var') && ~isempty(vecTraceAct)))
			...
			export_fig([strFigPath strFileName 'ZetaITI' strRunType 'Resamp' num2str(intResampleNum) '.tif']);
			print(gcf, '-dpdf', [strFigPath strFileName 'ZetaITI' strRunType 'Resamp' num2str(intResampleNum) '.pdf']);
		end
		%pause
	end
	%save
	...
	%cellArea = cell(1,intNeurons);
	if boolSave
	save([strDataTargetPath 'ZetaITIData' strRunType 'Resamp' num2str(intResampleNum) '.mat' ],...
