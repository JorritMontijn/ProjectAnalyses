		...
		for intIter=1:intIters
			...
			for intStimTypeIdx=1:intParamNum
				...
				if boolAllZero
					...
					%{
					%squeeze trials from folds
					mHD = [];
					for intFold=1:size(matMahalDists,4);
						mHD = cat(3,mHD,matMahalDists(:,:,:,intFold,:));
					end
					mHD = squeeze(mHD);
					
					mHD = squeeze(xmean(matMahalDists,4));
					mHD11 = squeeze(mHD(1,1,:,:));
					mHD12 = squeeze(mHD(1,2,:,:));
					mHD21 = squeeze(mHD(2,1,:,:));
					mHD22 = squeeze(mHD(2,2,:,:));
					
					%%
					mHD = squeeze(xmean(matMahalDists,3));
					mHDMean = squeeze(xmean(mHD,3));
					mHDSD = squeeze(xstd(mHD,3));
					
					figure
					hold on
					for intActual=1:2
						for intTarget = 1:2
							errorbar(vecGroupSizes+(intActual-1)+intTarget/2,mHDMean(intActual,intTarget,:),mHDSD(intActual,intTarget,:))
						end
					end
					legend({'R1T1','R1T2','R2T1','R2T2'});
