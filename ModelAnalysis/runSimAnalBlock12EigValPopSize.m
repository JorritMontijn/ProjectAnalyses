...
if intType == 1
	...
	vecPopSize = [10 25 50 100 200 400 600 800 1000];
	%vecRunSims = [-3 -4 -5];%[1 21 22 23];
	vecRunSims = 5%[4 100 1 150];%[1 21 22 23];
...
for intLoadSim=vecRunSims
	clearvars -except matPC1 intLimX dblLimFracX vecPopSize vecDoShuff vecRunSims intLoadSim bool* vecRunAreas
	boolLoad = true;
	...
	strFigDir = ['D:\Data\Results\DifferentialCorrelations\Block' strBlockNr '\'];
	strDataDir = ['D:\Data\Results\DifferentialCorrelations\Data' strBlockNr '\'];
	...
	for intWithinArea=vecRunAreas
		...
		if ~boolAllZero
			...
			matL = zeros(numel(vecPopSize),max(vecPopSize));
			intIterMax=100;
			for intIter=1:intIterMax
			matTempL = nan(numel(vecPopSize),max(vecPopSize));
			for intPopSizeIdx=1:numel(vecPopSize)
				intPopSize = vecPopSize(intPopSizeIdx);
				matCov = cov(matX1(randperm(size(matX1,1),intPopSize),1:1200)');
				[matV,vecL] = eig(matCov,'vector'); %eigenvectors V
				[vecLsorted,vecReorder] = sort(vecL,'descend');
				matTempL(intPopSizeIdx,1:intPopSize) = vecLsorted;
			end
			matL = matL + matTempL;
			end
			matL = matL ./ intIterMax;
