...
function [plaidsPVact] = PlaidsPVactTuningData(orientationdata, contrastdata)
...
plaidsact.neuron   = sSesAggregate.PV;
plaidsact.structStim   = sSesAggregate.structStim;

sSesAggregate.structStim.Orientation = plaidsact.structStim.Orientation(1:240);
sSesAggregate.structStim.TrialNumber = plaidsact.structStim.TrialNumber(1:240);
sSesAggregate.structStim.Contrast = plaidsact.structStim.Contrast(1:240);
sSesAggregate.structStim.FrameOn = plaidsact.structStim.FrameOn(1:240);
sSesAggregate.structStim.FrameOff = plaidsact.structStim.FrameOff(1:240);
%% Plaid laden
...
plaidsPVact.activ = zeros(plaidsact.n_neuronen, plaidsact.n_trials);



    for i = 1:plaidsact.n_neuronen
        for k = 1:plaidsact.n_trials
            plaidsPVact.activ(i,k) = mean(plaidsact.neuron(i).dFoF(plaid.contrindex(k).FrameOn:plaid.contrindex(k).FrameOff));
        end
    end
plaidsPVact.activZ = zscore(plaidsPVact.activ,[],1); %z-scored at each trial to remove mean population activity
plaidsPVact.structStim   = sSesAggregate.structStim;
end
