...
for intMouse=1:8
	...
	for intPopulation = vecBlockTypes
		...
		vecSEHit = cellfun(@std,cellHitActPrefPop)./sqrt(cellfun(@numel,cellHitActPrefPop));
		vecMeanMiss = cellfun(@mean,cellMissActPrefPop);
		vecSEMiss = cellfun(@std,cellMissActPrefPop)./sqrt(cellfun(@numel,cellMissActPrefPop));
		
		vecMeanHitWP = cellfun(@mean,cellHitActWholePop);
		vecSEHitWP = cellfun(@std,cellHitActWholePop)./sqrt(cellfun(@numel,cellHitActWholePop));
		vecMeanMissWP = cellfun(@mean,cellMissActWholePop);
		vecSEMissWP = cellfun(@std,cellMissActWholePop)./sqrt(cellfun(@numel,cellMissActWholePop));
		
		vecMeanHitNP = cellfun(@mean,cellHitActNPPop);
		vecSEHitNP = cellfun(@std,cellHitActNPPop)./sqrt(cellfun(@numel,cellHitActNPPop));
		vecMeanMissNP = cellfun(@mean,cellMissActNPPop);
		vecSEMissNP = cellfun(@std,cellMissActNPPop)./sqrt(cellfun(@numel,cellMissActNPPop));
		...
		%get p-value 2-5
		[h,dblP_PP] = ttest2(cell2mat(cellHitActPrefPop(2:5)),cell2mat(cellMissActPrefPop(2:5)));
		[h,dblP_WP] = ttest2(cell2mat(cellHitActWholePop(2:5)),cell2mat(cellMissActWholePop(2:5)));
		[h,dblP_NPP] = ttest2(cell2mat(cellHitActNPPop(2:5)),cell2mat(cellMissActNPPop(2:5)));
		...
		if sParams.boolSavePlots
			...
			title(sprintf('t-test, p=%.3f',dblP_PP));
			...
			xlabel('Stimulus contrast (%)')
			title(sprintf('t-test, p=%.3f',dblP_WP));
			%ylim([-0.01 0.06])
			...
			xlabel('Stimulus contrast (%)')
			title(sprintf('t-test, p=%.3f',dblP_NPP));
			%ylim([-0.01 0.06])
		...
		for intContrastIndex=1:intContrasts
			...
			vecMetaHitPPE(intContrastIndex) = std(vecHeteroHitPP)/sqrt(length(vecHeteroHitPP));
			vecMetaMissPPY(intContrastIndex) = vecYPP(2);
			vecMetaMissPPE(intContrastIndex) = std(vecHeteroMissPP)/sqrt(length(vecHeteroMissPP));
			vecPNPP(intContrastIndex) = pNPP;
			vecMetaHitNPPY(intContrastIndex) = vecYNPP(1);
			vecMetaHitNPPE(intContrastIndex) = std(vecHeteroHitNPP)/sqrt(length(vecHeteroHitNPP));
			vecMetaMissNPPY(intContrastIndex) = vecYNPP(2);
			vecMetaMissNPPE(intContrastIndex) = std(vecHeteroMissNPP)/sqrt(length(vecHeteroMissNPP));
