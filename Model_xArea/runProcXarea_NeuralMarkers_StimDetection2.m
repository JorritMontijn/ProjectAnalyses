...
%get neuron numbers
intUseNeuronsV1 = numel(vecSelectNeuronsV1);
intUseNeuronsV2 = numel(vecSelectNeuronsV2);
%V1
[vecPopSyncV1,vecTimestampsV1,intJitterBins] = getPopulationSynchronization(cellSpikesV1V2(vecSelectNeuronsV1),dblJitter_ms,vecTimestamps);
%V2
...
%get spikes
vecSpikesV1 = sort(cell2mat(cellSpikesV1V2(vecSelectNeuronsV1)));
vecSpikesV2 = sort(cell2mat(cellSpikesV1V2(vecSelectNeuronsV2)));
...
for intWinOffset=1:intWindowOffsets
	...
	[dummy,vecMeans] = makeBins(vecTimestampsV1,vecPopSyncV1,vecWindowEdges+dblWindowOffset);
	vecWinSyncEpochDurV1(vecAssign) = vecMeans;
...
%% transform to Hz
vecWinSpikeHzV1 = (vecWinSpikeCountV1 / dblWindowSecs) / intUseNeuronsV1;
vecWinSpikeHzV2 = (vecWinSpikeCountV2 / dblWindowSecs) / intUseNeuronsV2;
vecWinSyncHzV1 = ((((vecWinSyncEpochDurV1)*(dblBinSizeSecs/dblWindowStepSecs))/dblWindowSecs) / intUseNeuronsV1) / intJitterBins;
...
if intUniques > 20
	dblSyncStep = 0.05;
	vecEdgesSyncV1 = 0:dblSyncStep:1;
else
...
for intIdxC = 1:intNumC
	...
	vecTheseEdgesCountV1 = dblMinCountV1:dblStepCountV1:dblMaxCountV1;
	...
	if intUniques > 20
		dblSyncStep = 0.05;
		vecTheseEdgesSyncV1 = 0:dblSyncStep:1;
	else
		vecValCounts = zeros(1,intUniques);
		for intU=1:intUniques
			vecValCounts(intU) = sum(vecValIdx==intU);
		...
		if isempty(intCUV),intCUV = intUniques;end
		dblSyncStep = vecUniques(2) - vecUniques(1);
		vecTheseEdgesSyncV1 = (vecUniques(1) - dblSyncStep/2):dblSyncStep:(vecUniques(intCUV) + dblSyncStep/2);
	end


	%% get counts
	%build matrix
	[matCounts,matValMeans] = makeBins2(vecTheseWinSpikeHzV1,vecTheseWinSyncHzV1,vecTheseWinSpikeHzV2,vecTheseEdgesCountV1,vecTheseEdgesSyncV1);
	matValMeans(1,1) = nan;
	matCounts(1,1) = 0;
	...
	figure,plotSyncMap(vecTheseEdgesCountV1,vecTheseEdgesSyncV1,matPlotMeans,dblWindow_ms);
	subplot(2,2,4)
