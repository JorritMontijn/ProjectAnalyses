%% creates nm decoding error figs, including pseudo pops
...
for intSubType=1:2
	...
	for intRecIdx=1:numel(vecRunRecs)
		...
		for intBlockIdx=1:numel(vecBlocksNM)
			...
			if dblStimDur < 9
				dblMovieDur = 20/(60/25); %new; actually 8.33
				intBinNr = 25;
			else
				dblMovieDur = 20; %old
				intBinNr = 25;
			...
			%% prep data
			%split by ori
			vecMovieFrames = (vecBinIdx/max(vecBinIdx))*360;
			
			%include?
			indUseCells = arrayfun(@(x) x.Violations1ms < 0.25 & abs(x.NonStationarity) < 0.25,sRec.sCluster(:));
			vecUseCells = find(indUseCells);
			
			[vecTrialTypes,vecUnique,vecCounts,cellSelect,vecRepetition] = val2idx(vecMovieFrames);
			...
			matUseDiag = diag(diag(true(intStimNr)));
			...
			vecMovieOri = vecUnique(vecTrialTypes);
			vecCtxOri = vecUnique(vecDecodedIndexCV_LR_Ctx);
			vecNotOri = vecUnique(vecDecodedIndexCV_LR_NOT);
			
			vecCtxError = roundi(rad2deg(circ_dist(deg2rad(vecMovieOri),deg2rad(vecCtxOri))),6);
			vecNotError = roundi(rad2deg(circ_dist(deg2rad(vecMovieOri),deg2rad(vecNotOri))),6);
			vecCtxError(vecCtxError==180)=-180;
			vecNotError(vecNotError==180)=-180;
			
			vecErrorIdx = roundi(sort(unique([-(0:median(diff(vecUnique)):179) 0:median(diff(vecUnique)):179])),6);
			...
			%save fig
			drawnow;
			export_fig([strTargetPath filesep 'single_recs' filesep sprintf('MovieDecodingErrors%sB%d.tif',strName,intBlock)]);
			saveas(gcf,[strTargetPath filesep 'single_recs' filesep sprintf('MovieDecodingErrors%sB%d.pdf',strName,intBlock)]);
...
%save plot
drawnow;
export_fig([strTargetPath filesep sprintf('OriDecodingErrorsNM.tif')]);
saveas(gcf,[strTargetPath filesep sprintf('OriDecodingErrorsNM.pdf')]);
