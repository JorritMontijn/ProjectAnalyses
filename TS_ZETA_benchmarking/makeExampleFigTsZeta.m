...
for boolDoOGB = [false true]
	...
	for intFile=4%1:numel(cellRunRecs)
		...
		%% analyze
		hTic = tic;
		%20131022xyt01_ses N43
		for intNeuron=43%1:intNeurons%[1:intNeurons]%43%1:27, 2:69
			%% message
			if toc(hTic) > 5
			...
			dblPreUse = 2*-dblUseMaxDur*((dblJitterSize)/2);
			dblPostUse = 2*dblUseMaxDur*((dblJitterSize+1)/2);
