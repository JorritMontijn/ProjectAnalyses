function matSynchrony = calcSynchrony(cellSpikeTimes,vecWindow,numTau,dblStepSize)
	...
	%		calculation window
	%	[numTau]: temporal kernel width for calculating the synchrony metric;
	%		Default: dblTau = 0.01
	...
	%		synchronization values. The diagonal is normalized to be
	%		exactly the number of events (e.g., spike count) iff all events
	%		are regularly interspersed in time such that their temporal
	%		summation becomes negligeable based on the kernel width numTau
	%
	%Note: this function can in two ways, depending on whether you supplied
	%integer-type input or floating point input. The integer-based version
	%is considerably faster (around ~50%) than the float-based version.
	%
	...
	if boolInt
		...
		for intN1=1:intNumN
			...
			vecSyncs = nan(1,intNumN2);
			for intN2=1:intNumN2
				vecTimes2 = cellSpikeTimesSmall{intN2+intN1-1};
				matDiffs = bsxfun(@minus,vecTimes1,vecTimes2);
				dblSync = sum(sum(vecLookupTable(matDiffs(matDiffs>-1)+1)));
				vecSyncs(intN2) = dblSync;
			end
			matSynchrony(intN1,vecN2s) = vecSyncs;
			matSynchrony(vecN2s,intN1) = vecSyncs;
		...
		for intN1=1:intNumN
			...
			for intN2=1:intNumN2
				vecTimes2 = cellSpikeTimesSmall{intN2+intN1-1};
				dblSync = sum(sum(normpdf(bsxfun(@minus,vecTimes1,vecTimes2),0,numTau)));
				vecSyncs(intN2) = dblSync;
			end
			matSynchrony(intN1,vecN2s) = vecSyncs;
			matSynchrony(vecN2s,intN1) = vecSyncs;
