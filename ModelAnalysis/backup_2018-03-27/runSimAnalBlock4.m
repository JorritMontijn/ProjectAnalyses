...
for intLoadSim=vecRunSims
	...
	%% set parameters
	intUseRepetitionMax = inf;%400*10;
	intSizeX = 80;
	intSizeY = 30;
	boolSaveFigs = true;
	boolSaveData = true;
	...
	for intWithinArea=[3]
		...
		for intStimTypeIdx=1:intParamNum
			...
			%% subtract PSTH 
			%subtract PSTH somehow, but unclear how to proceed... subtract
			%for RRR, and then put it back for information calculation?
			if boolLoadTrialSplit
				error
			end
			...
			if boolAnalInfoSub
				if ~boolAllZero
					...
					sPredAndI = doDimPredAndI_RR(cellMatX,cellMatY,dblLambdaInfo,dblDiffTheta);
					...
					set(h,'Location','Best');
					title(sprintf('Pred %s activity; %s',strPredArea,strType));
					...
					if boolSaveFigs
						...
						export_fig([strFigDir  'AnalBlock4Subsp_Area' num2str(intWithinArea) '_' strSizeXY strSizeT strTag '.tif']);
						export_fig([strFigDir  'AnalBlock4Subsp_Area' num2str(intWithinArea) '_' strSizeXY strSizeT strTag '.pdf']);
					end
				else
		...
		%% save data
		if boolSaveData
			strDataFile = ['D:\Data\Processed\V1_LIFmodel\Simulation_' strSimulation '_AB4_Area' num2str(intWithinArea) strSizeXY strSizeT '.mat'];
