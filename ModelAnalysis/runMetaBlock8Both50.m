...
title(['f'' in \Sigma, top50&merged, \phi ' sprintf('(AUC) up to subspace rank %d; mean=%.3f; t-test vs 0,p=%.6f; t-test vs baseline (%.3f),p=%.6f',intMaxDim,mean(vecMean),dblP_zero,dblBaseMean,dblP_base)]);
errorbar((1:intN)-0.1,vecMean,vecSD,'xb');
errorbar((1:intN)+0.1,vecMeanMerge,vecSDMerge,'xr');
legend({'DiCo=0','DiCo=behav','DiCo=behav,merge','DiCo=top50','DiCo=merge'},'Location','best');
